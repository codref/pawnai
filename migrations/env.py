...
Reads the database DSN from (in priority order):
  1. ``paths.db_dsn`` in ``.pawn-diarize.yml`` (project config file)
  2. ``DATABASE_URL`` environment variable
  3. Hard-coded default (``postgresql+psycopg://postgres:postgres@localhost:5432/pawn_diarize``)

Wires the SQLAlchemy metadata from ``pawn_diarize.core.database.Base`` so that
